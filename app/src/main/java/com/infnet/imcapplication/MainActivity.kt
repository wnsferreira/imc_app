...
class MainActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnCalcular.setOnClickListener {
            val imcObject = Imc(
                editTxtPeso.text.toString(),
                editTxtAltura.text.toString()
            )
            val msg = "O preenchimento de todos os campos são necessários"

            if (imcObject.peso.isEmpty() or imcObject.altura.isEmpty()) {
                Toast.makeText(this, msg, Toast.LENGTH_LONG).show()
            } else {
                val imc = Math.round(imcObject.calcularIMC(imcObject.peso, imcObject.altura)).toDouble()
                editTxtIMC.text = "${imc.toString()} kg/m²"
                textViewGrauObesidade.text = imcObject.grauIMC(imc)
                card_view.visibility = View.VISIBLE
            }
        }
    }

//    public fun calcularIMC(peso: String, altura: String): Double =
//        peso.toDouble() / (altura.toDouble() * altura.toDouble())
//
//    public fun grauIMC(indice: Double): String {
//        return when (indice) {
